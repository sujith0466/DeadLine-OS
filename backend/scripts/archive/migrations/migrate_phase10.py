        ...
        for table in tables:
            ...
            try:
                ...
                with db.engine.connect() as conn:
                    ...
                    conn.execute(
                        text(
                            f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS user_id VARCHAR(36)"
                        )
                    )
                    ...
                    # 3. Alter to NOT NULL
                    conn.execute(
                        text(f"ALTER TABLE {table} ALTER COLUMN user_id SET NOT NULL")
                    )

                    # 4. Add foreign key if not exists (Postgres specific check)
                    fk_name = f"fk_{table}_user"
                    fk_check = conn.execute(
                        text(f"SELECT 1 FROM pg_constraint WHERE conname = '{fk_name}'")
                    ).fetchone()
                    if not fk_check:
                        conn.execute(
                            text(
                                f"ALTER TABLE {table} ADD CONSTRAINT {fk_name} FOREIGN KEY (user_id) REFERENCES users(id)"
                            )
                        )
