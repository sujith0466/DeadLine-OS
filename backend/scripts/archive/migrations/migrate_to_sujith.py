...
tables = [
    ...
    "twin_simulation_logs",
    "orchestrator_events",
]

with engine.connect() as conn:
    old_user = conn.execute(
        sqlalchemy.text("SELECT id FROM users WHERE username = 'sujith'")
    ).fetchone()
    ...
    if old_user_id and old_user_id != user_id:
        conn.execute(
            sqlalchemy.text(
                "UPDATE users SET username = 'sujith_old', email = 'old@deadlineos.com' WHERE id = :uid"
            ),
            {"uid": old_user_id},
        )

    # Insert new user
    existing_new_user = conn.execute(
        sqlalchemy.text("SELECT id FROM users WHERE id = :uid"), {"uid": user_id}
    ).fetchone()
    ...
    for table in tables:
        try:
            if old_user_id:
                conn.execute(
                    sqlalchemy.text(
                        f"UPDATE {table} SET user_id = :new_uid WHERE user_id = :old_uid"
                    ),
                    {"new_uid": user_id, "old_uid": old_user_id},
                )
            # Also catch any nulls or test-user-id if needed
            conn.execute(
                sqlalchemy.text(
                    f"UPDATE {table} SET user_id = :new_uid WHERE user_id IS NULL OR user_id = 'test-user-id' OR user_id = 'test-user-a-1234-5678'"
                ),
                {"new_uid": user_id},
            )
    ...
    if old_user_id and old_user_id != user_id:
        conn.execute(
            sqlalchemy.text("DELETE FROM users WHERE id = :uid"), {"uid": old_user_id}
        )
