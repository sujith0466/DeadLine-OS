...
class InterventionEngine:
    ...
    def run_engine(cls, user_id: str = None) -> List[Dict[str, Any]]:
        ...
        overdue_tasks = Task.query.filter(
            Task.user_id == uid, Task.status == "pending", Task.deadline < now
        ).all()
        ...
        if avail.get("utilization_percentage", 0) > 95:
            if "capacity_overload" not in active_types:
                inv = Threat(
                    ...
                    details={
                        "utilization_percentage": avail.get("utilization_percentage")
                    },
                )
                new_threats.append(inv)
        ...
        at_risk_habits = Habit.query.filter(
            Habit.user_id == uid, Habit.current_streak < 1
        ).all()
        ...
        TelemetryService.log_execution(
            "Threat Detection Engine", "Run Evaluation", "success", t0, 95
        )

        active = (
            Threat.query.filter_by(user_id=uid, status="active")
    ...
    def get_active_threats(
        cls, user_id: str = None, page: int = 1, limit: int = 100
    ) -> List[Dict[str, Any]]:
