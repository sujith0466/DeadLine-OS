...
class AvailabilityService:
    ...
    def get_current_availability(cls) -> Dict[str, Any]:
        ...
        today_events = [
            ...
            for e in events
            if e.get("start")
            and datetime.fromisoformat(e["start"].replace("Z", "+00:00")).date()
            == now.date()
        ]
        ...
        focus_hours = sum(
            [
                cls._calculate_hours(e)
                for e in today_events
                if e.get("type") == "focus_block"
            ]
        )
        ...
        pending_hours = sum(
            [t.estimated_hours for t in pending_tasks if t.estimated_hours]
        )
        ...
        return {
            ...
            "utilization_percentage": (
                round(min(100, (pending_hours / baseline_weekly) * 100))
