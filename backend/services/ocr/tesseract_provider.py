...
class TesseractProvider(OCRProvider):
    ...
    def __init__(self):
        ...
        if not self.is_available:
            logger.warning(
                "Tesseract executable not found on system. Tesseract OCR will be skipped."
            )

    def extract_text(
        self, image_bytes: bytes, cv_image: np.ndarray
    ) -> Tuple[str, float]:
        if not self.is_available:
            return "", 0.0
        ...
        try:
            ...
            data = pytesseract.image_to_data(
                cv_image, output_type=pytesseract.Output.DICT, config="--oem 3 --psm 6"
            )
