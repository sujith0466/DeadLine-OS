...
class GoalService:
    ...
    @classmethod
    def get_goals(
        cls, user_id: str, page: int = 1, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def get_habits(
        ...
        offset = (page - 1) * limit
        habits = (
            Habit.query.options(selectinload(Habit.logs))
            .filter_by(user_id=user_id)
            .offset(offset)
            .limit(limit)
            .all()
    ...
    @classmethod
    def create_goal(
        cls, user_id: str, title: str, description: str, category: str, target_date: str
    ) -> Dict[str, Any]:
        ...
        for idx, m_title in enumerate(agent_data.get("milestones", [])):
            ...
            if not m_user_id:
                raise ValueError(
                    "Milestone creation failed: a valid user_id is strictly required to satisfy database constraints."
                )
            ...
            deadline_date = dt.datetime.now(timezone.utc) + dt.timedelta(
                days=(idx + 1) * 7
            )
            task = Task(
                user_id=user_id,
    ...
    @classmethod
    def edit_goal(
        cls, user_id: str, goal_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def update_milestone_status(
        cls, user_id: str, milestone_id: str, status: str
    ) -> Dict[str, Any]:
        m = Milestone.query.get(
            milestone_id
        )  # Should theoretically join with Goal to check user_id, but Milestone assumes goal exists. We'll verify goal ownership.
        if not m:
    ...
    @classmethod
    def create_habit(
        cls, user_id: str, name: str, category: str, frequency: str
    ) -> Dict[str, Any]:
        threshold_time = datetime.now(timezone.utc) - dt.timedelta(seconds=30)
        duplicate = Habit.query.filter(
            Habit.user_id == user_id,
            Habit.name == name,
            Habit.created_at >= threshold_time,
        ).first()
        if duplicate:
            raise ValueError(f"Duplicate Request: Habit '{name}' was recently created.")

        habit = Habit(
            user_id=user_id, name=name, category=category, frequency=frequency
        )
    ...
    @classmethod
    def edit_habit(
        cls, user_id: str, habit_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "reminder_schedule" in data:
            habit.reminder_schedule = data["reminder_schedule"]
        if "target_duration" in data:
            habit.target_duration = data["target_duration"]
    ...
    def set_habit_status(
        cls, user_id: str, habit_id: str, status: str
    ) -> Dict[str, Any]:
        habit = Habit.query.filter_by(user_id=user_id, id=habit_id).first()
        if not habit:
            raise ValueError("Habit not found")
    ...
    def check_in_habit(cls, user_id: str, habit_id: str) -> Dict[str, Any]:
        ...
        existing_log = HabitLog.query.filter_by(
            habit_id=habit_id, date=today_str
        ).first()
        ...
        log = HabitLog(
            user_id=user_id, habit_id=habit_id, date=today_str, completed=True
        )
        db.session.add(log)
