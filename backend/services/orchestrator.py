...
class OrchestratorService:
    ...
    def add_event(
        ...
        event = OrchestratorEvent(
            user_id=uid, agent=agent, action=action, status=status, payload=payload
        )
        try:
            ...
            if count > 500:
                oldest = (
                    OrchestratorEvent.query.filter_by(user_id=uid)
    ...
    def run_pipeline(
        self, image_bytes: bytes, mime_type: str, availability: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        def _log(agent, action, status, data=None):
            ...
            trace.append(
                {"agent": agent, "action": action, "status": status, "data": data}
            )
        ...
        _log(
            "Vision Agent",
            f"Extracted {len(extracted_tasks)} tasks",
            "success",
            vision_result,
        )
        ...
        if not extracted_tasks:
            return {
                "status": "error",
                "message": "No tasks found in image.",
                "trace": trace,
            }
        ...
        TelemetryService.log_execution(
            "Planning Agent", "Schedule Synthesis", "success", t0, 85
        )
        ...
        accountability_result = self.accountability.generate_metrics(
            prioritized_tasks, [], []
        )
        _log(
        ...
        TelemetryService.log_execution(
            "Accountability Agent", "Behavior Analysis", "success", t0, 80
        )
        ...
        t0 = time.time()
        coach_result = self.coach.generate_coaching(
            prioritized_tasks, accountability_result
        )
        _log(
            "Coach Agent",
            "Generated personalized coaching plan",
            "success",
            coach_result,
        )
        TelemetryService.log_execution(
            "Coach Agent", "Coaching Insights", "success", t0, 88
        )
        ...
        t0 = time.time()
        rescue_result = self.rescue.generate_recovery_plan(
            prioritized_tasks, availability
        )

        if rescue_result.get("risk_detected"):
            _log(
                "Rescue Agent",
                "Detected high risk - generated recovery plan",
                "warning",
                rescue_result,
            )
        else:
            _log(
                "Rescue Agent",
                "Schedule is safe. No intervention needed.",
                "success",
                rescue_result,
            )
        ...
        TelemetryService.log_execution(
            ...
            t0,
            92,
            {"risk_detected": rescue_result.get("risk_detected")},
        )
        ...
        _log(
            "Digital Twin",
            "Simulating worst-case scenario (Delay critical task)",
            "running",
        )
        t0 = time.time()
        ...
        twin_result = self.twin.simulate_scenario(
            prioritized_tasks, scenario, availability
        )
        ...
        t0 = time.time()
        reflection_result = self.reflection.generate_reflection(
            prioritized_tasks, twin_result
        )
        _log(
            "Reflection Agent",
            "Generated daily reflection report",
            "success",
            reflection_result,
        )
        TelemetryService.log_execution(
            "Reflection Agent", "Daily Reflection", "success", t0, 85
        )
    ...
    def evaluate_system_state(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            for task in tasks_list:
                ...
                prioritized_tasks.append(task)
            _log(
                "Priority Agent", "Priority Agent SUCCESS", "success", prioritized_tasks
            )
            TelemetryService.log_execution(
                "Priority Agent", "System Priority Scoring", "success", t0, 95
            )
        except Exception as e:
            import traceback
        ...
        try:
            ...
            TelemetryService.log_execution(
                "Planning Agent", "System Schedule Synthesis", "success", t0, 90
            )
        except Exception as e:
            import traceback
        ...
        try:
            ...
            TelemetryService.log_execution(
                "Rescue Agent", "System Risk Detection", "success", t0, 92
            )
        except Exception as e:
            import traceback
        ...
        try:
            critical_task = max(
                prioritized_tasks, key=lambda x: x.get("priority_score", 0)
            )
            scenario = {
                "action": "delay_task",
                "task": critical_task["title"],
            ...
            TelemetryService.log_execution(
                "Digital Twin Agent", "System Simulation", "success", t0, 88
            )
        except Exception as e:
            import traceback
        ...
        try:
            if rescue_result.get("risk_detected"):
                reason = rescue_result.get(
                    "recovery_plan", "High risk detected in current workload."
                )
            ...
            _log(
                "Intervention Engine",
                "Intervention Engine SUCCESS",
                "success",
                {"interventions_generated": generated_count},
            )
            TelemetryService.log_execution(
                "Intervention Engine",
                "System Intervention Generation",
                "success",
                t0,
                90,
            )
            ...
            _log(
                "Intervention Engine",
                "Intervention Engine FAILED",
                "error",
                {"traceback": err},
            )
            return {
                "status": "error",
                "failed_stage": "Intervention Engine",
                "trace": trace,
            }
