...
def navigate_executor(
    ...
    return {
        ...
        "route": "/dashboard",
        "status": "Navigation requested",
        "message": "I'm not sure which page that is, opening your Dashboard.",
    }


def focus_mode_executor(
    entities: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
    return {
        "action": "navigate",
...
def task_service_executor(
    ...
    if existing_task:
        OrchestratorService.add_event(
            "Local Intelligence",
            "Prevented duplicate task creation",
            "warning",
            {"title": task_title},
        )
        return {
        ...
        db.session.commit()
        OrchestratorService.add_event(
            "Local Intelligence", "Created Task", "success", {"task_id": t.id}
        )
        return {
            "action": "create_task",
            "status": "Task created",
            "message": f"I've added {task_title} to your tasks.",
        }


def goal_service_executor(
    entities: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    res = GoalService.create_goal(
        ...
        "General",
        entities.get("target_date"),
    )
    OrchestratorService.add_event(
        "Local Intelligence",
        "Created Goal",
        "success",
        {"goal_id": res.get("goal", {}).get("id")},
    )
    return {
        "action": "create_goal",
        "status": "Goal created",
...
def rescue_agent_executor(
    ...
    uid = context.get("user_id")
    tasks = [
        t.to_dict() for t in Task.query.filter_by(user_id=uid, status="pending").all()
    ]
    execution_data = RescueAgent(context.get("gemini_service")).detect_risk(
        tasks, {"daily_available_hours": 4}
    )
    OrchestratorService.add_event(
        "Local Intelligence",
        "Triggered Rescue Mode",
        "warning",
        {"risk_detected": execution_data.get("risk_detected")},
    )
    return {
        "action": "rescue_analysis",
        "status": "Analysis started",
        "message": "I've started a rescue analysis.",
    }


def digital_twin_executor(
    entities: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    DigitalTwinAgent(context.get("gemini_service")).simulate_scenario(
        tasks,
        {"action": "shift", "shift": "Local Intelligence Simulation"},
        {"daily_available_hours": 8},
    )
    OrchestratorService.add_event(
        "Local Intelligence",
        "Triggered Digital Twin",
        "success",
        {"scenario": "Simulation"},
    )
    return {
...
def planner_agent_executor(
    ...
    PlanningAgent(context.get("gemini_service")).generate_schedule(
        tasks, {"daily_available_hours": 8}
    )
    OrchestratorService.add_event(
        "Local Intelligence",
        "Generated Schedule",
    ...
    return {
        ...
        "message": "I've planned your schedule.",
    }


def general_query_executor(
    entities: Dict[str, Any], context: Dict[str, Any]
) -> Dict[str, Any]:
    from services.orchestrator import OrchestratorService
