...
class EntityExtractor:
    ...
    def extract(cls, transcript: str, intent: str) -> Dict[str, Any]:
        ...
        task_match = re.search(
            r"(?:add|create)(?:\s+a)?(?:\s+task)?\s+(?:to|for)?\s*(.*?)(?:\s+for|\s+tomorrow|\s+today|\s+next|$)",
            transcript,
            re.IGNORECASE,
        )
        if (
            task_match
            and task_match.group(1).strip()
            and task_match.group(1).strip().lower() not in ["task", "goal"]
        ):
            entities["target_name"] = task_match.group(1).strip()

        goal_match = re.search(
            r"(?:add|create)(?:\s+a)?(?:\s+goal)?\s*(?:to|for)?\s*(.*?)(?:\s+by|\s+for|$)",
            transcript,
            re.IGNORECASE,
        )
        if (
            goal_match
            and not "target_name" in entities
            and goal_match.group(1).strip().lower() not in ["goal", "task"]
        ):
            entities["target_name"] = goal_match.group(1).strip()

        # Navigation targets
        nav_match = re.search(
            r"(?:open|show|go to)\s+(dashboard|settings|goals|planner|calendar|rescue|analytics|documents|vision)",
            text_lower,
        )
        if nav_match:
        ...
        doc_match = re.search(
            r"(?:analyze|read|process)\s+(?:this\s+)?(?:document|file)\s+([a-zA-Z0-9_\-\.]+)",
            text_lower,
        )
        if doc_match:
        ...
        image_match = re.search(
            r"(?:analyze|what is in)\s+(?:this\s+)?(?:image|picture|photo)\s+([a-zA-Z0-9_\-\.]+)",
            text_lower,
        )
        if image_match:
        ...
        dates_found = search_dates(
            transcript, settings={"RELATIVE_BASE": None, "PREFER_DATES_FROM": "future"}
        )
        if dates_found:
        ...
        project_match = re.search(
            r"in\s+(?:the\s+)?([a-zA-Z0-9\s]+)\s+project", text_lower
        )
        if project_match:
