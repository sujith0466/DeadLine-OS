...
class ExecutionEngine:
    ...
    @classmethod
    def execute(
        cls, source: str, transcript: str, gemini_service, user_id: str = None
    ) -> Dict[str, Any]:
        t0 = time.time()
        ...
        if intent == "unknown" or confidence < 70:
            ...
            agent = VoiceCopilotAgent(
                gemini_service
            )  # Eventually abstract this to a general Copilot Agent
            ...
            LearningService.log_command(
                ...
                confidence,
                source,
                "gemini_fallback" if intent != "unknown" else "unknown",
            )
        elif confidence < 90:
