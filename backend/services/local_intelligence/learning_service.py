...
class LearningService:
    ...
    def sanitize_transcript(cls, text: str) -> str:
        ...
        text = re.sub(
            r"(?i)(password|pass|jwt|token|secret)[\s:=]+[\w\-._]+",
            r"\1 [REDACTED]",
            text,
        )
        return text
    ...
    def log_suggestion_outcome(
        cls, user_id: str, suggestion_id: str, source: str, accepted: bool
    ):
        try:
    ...
    @classmethod
    def log_correction(
        cls, user_id: str, original_transcript: str, corrected_intent: str, source: str
    ):
        try:
