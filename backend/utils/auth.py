...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth_header or not auth_header.startswith("Bearer "):
            ...
            raise APIError(
                "Missing Authorization header", code="UNAUTHORIZED", status=401
            )
        ...
        try:
            ...
            if alg == "HS256":
                ...
                if not secret:
                    raise APIError(
                        "Auth configuration missing", code="INTERNAL_ERROR", status=500
                    )
                payload = jwt.decode(
        ...
        if not user:
            logger.info(
                f"First time login for user {user_id}. Syncing identity to Neon DB."
            )
            ...
            try:
                ...
                raise APIError(
                    "Failed to sync user identity", code="INTERNAL_ERROR", status=500
                )
        ...
        if email and email.startswith("demo_") and "@deadlineos.com" in email:
            ...
            user
            and user.email
            and user.email.startswith("demo_")
            and "@deadlineos.com" in user.email
        ):
            is_demo_email = True
