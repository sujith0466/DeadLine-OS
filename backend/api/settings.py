...
def update_profile():
    ...
    # Validate timezone before committing — an invalid IANA tz corrupts all scheduling
    if "timezone" in data:
        from utils.timezone import validate_iana_timezone

        if not validate_iana_timezone(data["timezone"]):
            raise APIError(
                f"Invalid timezone '{data['timezone']}'. Must be a valid IANA timezone string (e.g. 'Asia/Kolkata', 'America/New_York').",
...
def get_sessions():
    sessions = (
        UserSession.query.filter_by(user_id=g.user_id)
...
def delete_session(session_id):
    ...
    if not session:
        raise APIError("Session not found", status=404)
