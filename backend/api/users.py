...
class UserProfileUpdateSchema(Schema):
    ...
    focus_hours_start = fields.String(
        validate=validate.Regexp(r"^\d{2}:\d{2}$"), allow_none=True
    )
    focus_hours_end = fields.String(
        validate=validate.Regexp(r"^\d{2}:\d{2}$"), allow_none=True
    )
    daily_capacity = fields.Float(validate=validate.Range(min=0.5, max=24.0))
    preferred_planning_mode = fields.String(
        validate=validate.OneOf(["balanced", "aggressive", "safe"])
    )
    theme_preference = fields.String(
...
def update_my_profile(validated_data):
    ...
    try:
        ...
        db.session.rollback()
        raise APIError(
            "Failed to update profile due to a conflict.",
            status=409,
            details={"error": str(e)},
        )
