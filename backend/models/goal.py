...
class Milestone(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_milestone_user"),
        nullable=True,
        index=True,
    )
...
class Habit(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_habit_user"),
        nullable=True,
        index=True,
    )
    ...
    )  # 0-100 metric tracking consistency velocity
    created_at = db.Column(
        db.DateTime, nullable=False, default=lambda: datetime.now(timezone.utc)
    )
    status = db.Column(db.String(50), default="Active")
    ...
    logs = db.relationship(
        "HabitLog", backref="habit_ref", lazy=True, cascade="all, delete-orphan"
    )
...
class HabitLog(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_habitlog_user"),
        nullable=True,
        index=True,
    )
