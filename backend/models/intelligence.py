...
class AccountabilityMetrics(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_accountability_user"),
        nullable=True,
        index=True,
    )
...
class CoachReport(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_coachreport_user"),
        nullable=True,
        index=True,
    )
...
class ReflectionReport(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_reflection_user"),
        nullable=True,
        index=True,
    )
...
class ExecutionProfile(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_executionprofile_user"),
        nullable=True,
        index=True,
    )
...
class WeeklyReview(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_weeklyreview_user"),
        nullable=True,
        index=True,
    )
    created_at = db.Column(
...
class CommandLog(db.Model):
    ...
    user_id = db.Column(
        db.String(36),
        db.ForeignKey("users.id", name="fk_commandlog_user"),
        nullable=True,
        index=True,
    )
    source = db.Column(
        db.String(50), nullable=False
    )  # e.g., "voice", "vision", "document"
    transcript = db.Column(db.Text, nullable=False)
    detected_intent = db.Column(db.String(100), nullable=True)
    confidence_score = db.Column(db.Float, default=0.0)
    execution_outcome = db.Column(
        db.String(100), nullable=True
    )  # e.g., "success", "gemini_fallback", "user_corrected"
