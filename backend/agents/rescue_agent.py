...
class RescueAgent:
    ...
    def generate_recovery_plan(
        ...
        def _local_inference():
            # Sort tasks by size
            sorted_tasks = sorted(
                tasks, key=lambda x: float(x.get("estimated_hours", 1.0)), reverse=True
            )
            ...
            c2_scenario = {
                "action": "REDUCE_HOURS",
                "hours": 4,
            }  # simulate heavy reduction to force deferral
            ...
            c4_scenario = {
                "action": "DELAY_TASK",
                "task_id": (
                    second_largest.get("id")
                    if second_largest
                    else largest_task.get("id")
                ),
                "delay_days": 1,
            }
            ...
            strategies = [
                {
                    "name": "Safe",
                    "success_prob": c1_res.get("projected_state", {}).get(
                    ...
                    "actions": [
                        {
                            ...
                            "task_id": largest_task.get("id"),
                            "delay_days": 2,
                            "description": f"Defer '{largest_task.get('title')}' by 2 days.",
                        },
                        {
                            "action": "PAUSE_HABITS",
                            "description": "Pause non-essential habits.",
                        },
                    ],
                ...
                {
                    "name": "Balanced",
                    "success_prob": c3_res.get("projected_state", {}).get(
                    ...
                    "actions": [
                        {
                            ...
                            "task_id": largest_task.get("id"),
                            "delay_days": 1,
                            "description": f"Defer '{largest_task.get('title')}' by 1 day.",
                        },
                        {
                            "action": "MAINTAIN_HABITS",
                            "description": "Keep habits active.",
                        },
                    ],
                ...
                {
                    "name": "Aggressive",
                    "success_prob": c5_res.get("projected_state", {}).get(
                        "success_probability", 60
                    ),
                    "actions": [
                        {
                            "action": "INJECT_FOCUS",
                            "description": "Inject +2 hours of Focus Blocks.",
                        },
                        {
                            "action": "MAINTAIN_HABITS",
                            "description": "Keep habits active.",
