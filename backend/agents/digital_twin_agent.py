...
TWIN_SCHEMA = {
    ...
    "properties": {
        ...
        "cascade": {
            ...
            "items": {
                "type": "object",
                "properties": {"step": {"type": "string"}, "desc": {"type": "string"}},
                "required": ["step", "desc"],
            },
        },
    ...
    def simulate_scenario(
        ...
        if not tasks and not scenario:
            return {
                ...
                "risk_factors": [],
                "recommendations": [
                    {"action": "You are free to take on new work.", "confidence": 100}
                ],
                "success_probability": 100,
        ...
        def _local_inference():
            ...
            if action == "DELAY_TASK":
                ...
                for t in cloned_tasks:
                    if t.get("id") == task_id or t.get("title") == scenario.get(
                        "title"
                    ):
                        ...
                        if dl_str and dl_str != "No deadline":
                            try:
                                ...
                                t["deadline"] = (
                                    dl + datetime.timedelta(days=delay_days)
                                ).isoformat()
                            except Exception:
                                pass
                ...
                cloned_availability["daily_available_hours"] = max(
                    1.0, hrs - float(scenario.get("hours", 2))
                )
            else:
            ...
            capacity_impact = max(
                0, (projected_backlog_len - baseline_backlog_len) * 20
            )
            ...
            if projected_prob < baseline_prob:
                recs.append(
                    {
                        "action": f"Defer low priority tasks to restore {baseline_prob - projected_prob}% success rate.",
                        "confidence": 90,
                    }
                )
            if projected_backlog_len > baseline_backlog_len:
                recs.append(
                    {
                        "action": f"Split oversized tasks into 2 sessions to clear {projected_backlog_len - baseline_backlog_len} backlog items.",
                        "confidence": 85,
                    }
                )
            if not recs:
                recs.append(
                    {
                        "action": "Proceed with scenario. Schedule remains stable.",
                        "confidence": 95,
                    }
                )
            ...
            return {
                ...
                "cascade": [
                    {
                        "step": "Schedule Cloned",
                        "desc": f"Simulation initialized with {len(cloned_tasks)} tasks",
                    },
                    {"step": "Scenario Injected", "desc": f"Action: {action}"},
                    {
                        "step": "Engine Simulation",
                        "desc": f"Planner generated simulated schedule. Backlog Δ: {projected_backlog_len - baseline_backlog_len}",
                    },
                ],
                "risk_factors": [
                    (
                    ...
                    (
                        f"Backlog grew by {projected_backlog_len - baseline_backlog_len}"
                        if projected_backlog_len > baseline_backlog_len
                        else "No backlog impact"
                    ),
                ],
