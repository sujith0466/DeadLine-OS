...
VISION_SCHEMA = {
    ...
    "properties": {
        "tasks": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "deadline": {
                        "type": "string",
                        "description": "YYYY-MM-DD or 'None'",
                    },
                    "priority": {
                        "type": "string",
        ...
        "deadlines": {
            ...
            "items": {
                "type": "object",
                "properties": {"task": {"type": "string"}, "date": {"type": "string"}},
                "required": ["task", "date"],
            },
        },
...
class VisionAgent:
    ...
    def __init__(self, gemini_service):
        ...
        self.gemini_ocr = GeminiProvider(
            gemini_service, VISION_PROMPT_TEMPLATE, VISION_SCHEMA
        )
    ...
    def extract_tasks_via_ocr(self, image_bytes: bytes) -> tuple[Dict[str, Any], float]:
        ...
        try:
            ...
            if cv_image is None or not self.tesseract.is_available:
                logger.info(
                    "Tesseract unavailable or image corrupted. Using Gemini Fallback."
                )
                gemini_res = self.extract_tasks_from_image(image_bytes, "image/png")
                return {
                    "raw_text": gemini_res.get(
                        "summary", "Extracted via Gemini Vision"
                    ),
            ...
            if not is_reliable:
                logger.info(
                    "OCR deemed unreliable (conf: %.2f, len: %d, blur: %.1f). Falling back to Gemini.",
                    avg_conf,
                    text_len,
                    blur_score,
                )
            ...
            if task_text.lower() not in [
                ...
                if dateparser:
                    parsed_date_obj = dateparser.parse(
                        task_text, settings={"STRICT_PARSING": False}
                    )
                    if parsed_date_obj:
                ...
                if not parsed_date:
                    date_match = re.search(
                        r"(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2}|\d{4}-\d{2}-\d{2}",
                        task_text,
                        re.IGNORECASE,
                    )
                    if date_match:
    ...
    def extract_tasks_from_image(
        ...
        """
        logger.info(
            "Vision Agent analyzing %s image (%d bytes) with Gemini",
            mime_type,
            len(image_bytes),
        )
