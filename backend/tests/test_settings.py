...
def test_update_settings_section(client, mock_auth_headers):
    res = client.put(
        "/api/settings/appearance", headers=mock_auth_headers, json={"theme": "dark"}
    )
    assert res.status_code == 200
...
def test_get_sessions(client, mock_auth_headers, app):
    # Extract the user ID used in the mock_auth_headers
    token = mock_auth_headers["Authorization"].split(" ")[1]
    with app.app_context():
        session = UserSession(user_id=token, browser="Chrome", os="Mac")
