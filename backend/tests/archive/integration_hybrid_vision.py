        ...
        with app.app_context():
            ...
            for fname, text, noise in visions:
                ...
                if res.status_code == 200:
                    ...
                    print(
                        f"SUCCESS (Upload): Extracted raw text. Confidence: {resp_data.get('confidence')}"
                    )

                    print(f"Testing Confirm Phase for: {fname}")
                    confirm_payload = {"raw_text": raw_text, "confirmed_tasks": tasks}
                    res_confirm = client.post(
                        "/api/agents/vision/confirm", json=confirm_payload
                    )

                    if res_confirm.status_code == 200:
                        conf_data = res_confirm.get_json()
                        print(
                            f"SUCCESS (Confirm): Persisted {len(conf_data.get('inserted_task_ids', []))} tasks."
                        )
                    else:
            ...
            if res.status_code == 200:
                ...
                print(
                    f"Gemini Processed Images: {data.get('gemini_processed_images', 0)}"
                )
